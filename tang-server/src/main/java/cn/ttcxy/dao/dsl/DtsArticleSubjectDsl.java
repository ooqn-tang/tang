...
public class DtsArticleSubjectDsl {
  ...
  public DtsArticleSubjectDto findSubjectById(String subjectId) {
    return query
        .select(Projections.bean(DtsArticleSubjectDto.class, qArticleSubject.subjectId,
            qArticleSubject.subjectName, qArticleSubject.synopsis, qArticleSubject.createDate,
            qAuthor.nickname, qAuthor.username))
        .from(qArticleSubject, qAuthor).where(qAuthor.authorId.eq(qArticleSubject.authorId),
  ...
  public Page<DtsArticleSubjectDto> findSubjectList(Pageable pageable) {
    JPAQuery<?> jpaQuery =
        query.from(qArticleSubject, qAuthor).where(qAuthor.authorId.eq(qArticleSubject.authorId));
  ...
  public List<DtsArticleSubjectDto> findSubjectListByUsername(String username) {
    JPAQuery<?> jpaQuery = query.from(qArticleSubject).leftJoin(qAuthor)
        .on(qAuthor.authorId.eq(qArticleSubject.authorId)).where(qAuthor.username.eq(username));

    return jpaQuery.select(Projections.bean(DtsArticleSubjectDto.class, qArticleSubject.subjectId,
        qArticleSubject.subjectName, qArticleSubject.synopsis, qArticleSubject.createDate,
        qArticleSubject.updateDate, qArticleSubject.authorId, qAuthor.username, qAuthor.nickname))
        .fetch();
  }
  ...
  public String findSubjectIdByArticleId(String articleId) {
    return query.select(qSubjectRelevance.subjectId).from(qSubjectRelevance)
        .where(qSubjectRelevance.articleId.eq(articleId)).fetchOne();
  }

  public List<DtsArticle> findAllBySubjectId(String subjectId) {
    return query.select(qArticle).from(qArticle, qSubjectRelevance)
        .where(qArticle.articleId.eq(qSubjectRelevance.articleId),
            qSubjectRelevance.subjectId.eq(subjectId))
        .fetch();
  }

  public List<DtsArticle> findSubjectArticleTitleListBySubjectId(String subjectId) {
    return query.select(Projections.bean(DtsArticle.class, qArticle.articleId, qArticle.title))
        .from(qArticle, qSubjectRelevance).where(qArticle.articleId.eq(qSubjectRelevance.articleId),
            qSubjectRelevance.subjectId.eq(subjectId))
        .fetch();
  }

  public void deleteBySubjectIdAndAuthorId(String subjectId, String authorId) {
    query.delete(qArticleSubject)
        .where(qArticleSubject.authorId.eq(authorId), qArticleSubject.subjectId.eq(subjectId))
        .execute();
  }
}
