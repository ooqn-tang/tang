...
public class DtsArticleDsl {
  ...
  public Page<DtsArticleDto> selectArticleList(Pageable pageable) {
    JPAQuery<?> jpaQuery = query.from(qAuthor, qArticle)
    ...
    List<DtsArticleDto> articleList = jpaQuery
        .select(Projections.bean(DtsArticleDto.class, qArticle.articleId, qArticle.title,
            qArticle.createDate, qArticle.updateDate, qAuthor.username, qAuthor.nickname,
            qArticle.synopsis))
  ...
  public Page<DtsArticleDto> selectGzArticleList(String authorId, Pageable pageable) {
    ...
    JPAQuery<?> jpaQuery =
        query.from(qFans).leftJoin(qArticle).on(qFans.beAuthorId.eq(qArticle.authorId))
            .leftJoin(qAuthor).on(qFans.beAuthorId.eq(qAuthor.authorId))
  ...
  public Page<DtsArticleDto> selectArticleListSmall(Pageable pageable) {
    JPAQuery<?> jpaQuery = query.from(qArticle).where(qArticle.state.eq(1));

    Long count = jpaQuery.select(qArticle.articleId.count()).fetchOne();

    List<DtsArticleDto> articleList = jpaQuery
        .select(Projections.bean(DtsArticleDto.class, qArticle.articleId, qArticle.title,
            qArticle.synopsis, qArticle.createDate))
        .orderBy(qArticle.createDate.desc()).offset(pageable.getOffset())
        .limit(pageable.getPageSize()).fetch();
    return new PageImpl<>(articleList, pageable, count);
  }
  ...
  public Page<DtsArticleDto> search(String title, Pageable pageable) {
    JPAQuery<?> jpaQuery =
        query.from(qAuthor, qArticle).where(qAuthor.authorId.eq(qArticle.authorId),
            qArticle.state.eq(1), qArticle.title.like("%" + title + "%"));
  ...
  public DtsArticleDto selectArticleAllById(String articleId) {
    return query
        .select(Projections.bean(DtsArticleDto.class, qArticle.articleId, qAuthor.username,
            qAuthor.nickname, qArticle.title, qArticle.createDate, qArticle.updateDate,
            qArticle.synopsis, qArticle.text, qArticle.markdown))
        .from(qAuthor, qArticle)
  ...
  public Page<DtsArticleDto> selectArticleListByUsername(String username, Pageable pageable) {
    JPAQuery<?> jpaQuery = query.from(qAuthor).innerJoin(qArticle)
        ...
        .leftJoin(qSubjectRelevance).on(qSubjectRelevance.articleId.eq(qArticle.articleId))
    ...
    List<DtsArticleDto> fetch = jpaQuery
        .select(Projections.bean(DtsArticleDto.class, qArticle.articleId, qAuthor.username,
            qAuthor.nickname, qArticle.title, qArticle.createDate, qArticle.updateDate,
            qArticle.synopsis, qDtsArticleSubject.subjectName))
        .orderBy(qArticle.createDate.desc()).offset(pageable.getOffset())
        .limit(pageable.getPageSize()).fetch();
  ...
  public List<DtsArticleDto> selectArticleList1000() {
    return query.select(Projections.bean(DtsArticleDto.class, qArticle.articleId, qArticle.title))
