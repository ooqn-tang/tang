...
public class DtsFileController extends BaseController {
	...
	@GetMapping("{fileName}")
	public void create(@PathVariable("fileName") String fileName, HttpServletRequest request,
			...
			if (file.exists()) {
				...
				try (RandomAccessFile targetFile = new RandomAccessFile(file, "r")) {
					...
					if (rangeString != null) { // 如果rangeString不为空，证明是播放视频发来的请求
						long range = Long.parseLong(rangeString
								.substring(rangeString.indexOf("=") + 1, rangeString.indexOf("-")));
						// 设置内容类型
