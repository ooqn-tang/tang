  ...
  "node_modules/tinymce/plugins/table/plugin.js"() {
    (function() {
      ...
      const fireTableModified = (editor, table2, data) => {
        editor.dispatch("TableModified", __spreadProps(__spreadValues({}, data), {
          table: table2
        }));
      };
      ...
      const determineDefaultStyles = (editor, defaultStyles) => {
        if (isResponsiveForced(editor) || !shouldStyleWithCss(editor)) {
          ...
          return __spreadProps(__spreadValues({}, defaultStyles), {
      ...
      const determineDefaultAttributes = (editor, defaultAttributes) => {
        if (isResponsiveForced(editor) || shouldStyleWithCss(editor)) {
          ...
          return __spreadProps(__spreadValues({}, defaultAttributes), {
            width: getPixelForcedWidth(editor)
          });
        } else {
          return __spreadProps(__spreadValues({}, defaultAttributes), {
      ...
      const extractDataFromSettings = (editor, hasAdvTableTab) => {
        ...
        const getCellPaddingCellSpacing = () => {
          ...
          return __spreadValues(__spreadValues({}, spacing), padding);
        };
        const data = __spreadValues(__spreadValues(__spreadValues(__spreadValues(__spreadValues(__spreadValues({}, defaultData), style), attrs), advStyle), getBorder()), getCellPaddingCellSpacing());
        return data;
      };
      ...
      const applyDataToElement = (editor, tableElm, data) => {
        ...
        attrs.style = dom.serializeStyle(__spreadValues(__spreadValues({}, getDefaultStyles(editor)), styles));
        dom.setAttribs(tableElm, __spreadValues(__spreadValues({}, getDefaultAttributes(editor)), attrs));
      };
      ...
      const addButtons = (editor, selectionTargets) => {
        ...
        const addButtonIfRegistered = (name2, spec) => {
          if (editor.queryCommandSupported(spec.command)) {
            editor.ui.registry.addButton(name2, __spreadProps(__spreadValues({}, spec), {
        ...
        const addToggleButtonIfRegistered = (name2, spec) => {
          if (editor.queryCommandSupported(spec.command)) {
            editor.ui.registry.addToggleButton(name2, __spreadProps(__spreadValues({}, spec), {
              onAction: isFunction(spec.onAction) ? spec.onAction : cmd(spec.command)
            }));
          }
        };
      ...
      const addMenuItems = (editor, selectionTargets) => {
        ...
        const addMenuIfRegistered = (name2, spec) => {
          if (editor.queryCommandSupported(spec.command)) {
            editor.ui.registry.addMenuItem(name2, __spreadProps(__spreadValues({}, spec), {
              onAction: isFunction(spec.onAction) ? spec.onAction : cmd(spec.command)
            }));
            return true;
          } else {
        ...
        const addToggleMenuIfRegistered = (name2, spec) => {
          if (editor.queryCommandSupported(spec.command)) {
            editor.ui.registry.addToggleMenuItem(name2, __spreadProps(__spreadValues({}, spec), {
              onAction: isFunction(spec.onAction) ? spec.onAction : cmd(spec.command)
            }));
          }
        };
